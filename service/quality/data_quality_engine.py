...
class DataQuality:
    ...
    @staticmethod
    def exclude_faulty_feedback_view_point_elevation_v2(feedback_data):
        user_data = {}
        for entry in feedback_data:
            user_id = entry.user_id
            if user_id not in user_data:
                user_data[user_id] = {'verdict_view_0_count': 0, 'verdict_view_1_count': 0, 'verdict_view_2_count': 0}
            if entry.verdict_view_point_elevation == 0:
                user_data[user_id]['verdict_view_0_count'] += 1
            elif entry.verdict_view_point_elevation == 1:
                user_data[user_id]['verdict_view_1_count'] += 1
            else:
                user_data[user_id]['verdict_view_2_count'] += 1

        # Create features for anomaly detection
        features = []
        for user_id, counts in user_data.items():
            ratio = counts['verdict_view_1_count'] / (
                    counts['verdict_view_0_count'] + counts['verdict_view_1_count'] + counts['verdict_view_2_count'])
